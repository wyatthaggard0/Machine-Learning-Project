...
export async function POST(request) {
  ...
  try {
    ...
    if (!Array.isArray(features) || features.length !== 30) {
      return NextResponse.json({ error: "features must be an array of 30 numbers" }, { status: 400 })
    }
  } catch {
  ...
  try {
    ...
    return NextResponse.json({ error: "Prediction failed", detail: err.message }, { status: 502 })
  }
}
